...
pub async fn add_handler(Extension(app_data): Extension<Arc<AppData>>,
                         Extract(maybe_bearer_token): Extract,
                         Json(item_form): Json<AddItemForm>, ) -> Response {
    if let Some(desc) = &item_form.description {
        return match app_data.item_service.add_item_with_desc(&item_form.name, &desc, &item_form.sn).await {
...
pub async fn update_handler(Extension(app_data): Extension<Arc<AppData>>,
                            Extract(maybe_bearer_token): Extract,
                            Path(item_id): Path<ItemId>,
                            Json(item_form): Json<UpdateItemForm>, ) -> Response {
...
#[allow(clippy::unused_async)]
pub async fn get_handler(Extension(app_data): Extension<Arc<AppData>>,
                         Extract(maybe_bearer_token): Extract,
                         Path(item_id): Path<ItemId>, ) -> Response {
...
#[allow(clippy::unused_async)]
pub async fn transfer_handler(Extension(app_data): Extension<Arc<AppData>>,
                              Extract(maybe_bearer_token): Extract,
                              Json(item_form): Json<TransferItemForm>, ) -> Response {
    match app_data.item_service.transfer_item(&item_form.item_id,
                                              item_form.count,
                                              item_form.shelf_from,
                                              item_form.shelf_to).await {
...
#[allow(clippy::unused_async)]
pub async fn withdraw_handler(Extension(app_data): Extension<Arc<AppData>>,
                              Extract(maybe_bearer_token): Extract,
                              Path(item_id): Path<ItemId>,
                              Json(item_form): Json<ItemOnShelfForm>, ) -> Response {
    match app_data.item_service.withdraw_item(&item_id, item_form.count, item_form.shelf_id).await {
        Ok(_) => Json(OkResponseData { data: "todo" }).into_response(),
        Err(error) => error.into_response(),
    }
}
...
#[allow(clippy::unused_async)]
pub async fn deposit_handler(Extension(app_data): Extension<Arc<AppData>>,
                             Extract(maybe_bearer_token): Extract,
                             Path(item_id): Path<ItemId>,
                             Json(item_form): Json<ItemOnShelfForm>, ) -> Response {
    match app_data.item_service.deposit_item(&item_id, item_form.count, item_form.shelf_id).await {
        Ok(_) => Json(OkResponseData { data: "todo" }).into_response(),
        Err(error) => error.into_response(),
    }
}
...
#[allow(clippy::unused_async)]
pub async fn convert_handler(Extension(app_data): Extension<Arc<AppData>>,
                             Extract(maybe_bearer_token): Extract,
                             Json(item_form): Json<ConvertItemForm>, ) -> Response {
